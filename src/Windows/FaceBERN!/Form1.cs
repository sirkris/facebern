...
namespace FaceBERN_
{
    public partial class Form1 : Form
    {
        ...
        private void LoadCampaignsINI(string sectionName = "Campaigns")
        {
            string configDir = (Globals.ConfigDir != null ? Globals.ConfigDir : "");
            string configPath = Path.Combine(configDir, Globals.CampaignsINI);

            if (File.Exists(configPath) == false)
            {
                LogW("Campaigns config file '" + configPath + "' does not exist.  Creating....", false);

                Dictionary<string, Dictionary<string, string>> config = new Dictionary<string, Dictionary<string, string>>();

                config[sectionName] = new Dictionary<string, string>();

                // Values are initialized in Workflow.BirdieToCampaigns(), called from InterCOM thread at startup and periodically after that.  --Kris

                Globals.sINI.Create(configPath, "Birdie (FaceBERN!) Campaigns Configuration File", "Generated by Version " + Globals.__VERSION__, config);
            }

            Dictionary<string, string> conf = Globals.sINI.Load(configPath);

            Dictionary<int, bool> campaignConfigs = new Dictionary<int, bool>();

            foreach (KeyValuePair<string, string> pair in conf)
            {
                int n;
                if (pair.Key.Length > 11
                    && pair.Key.Substring(0, 11).Equals("UseCampaign") 
                    && int.TryParse( pair.Key.Substring( 11 ), out n ))
                {
                    campaignConfigs.Add(int.Parse(pair.Key.Substring(11)), pair.Value.Equals("1"));
                }
            }

            Globals.CampaignConfigs = campaignConfigs;
        }
